...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Roadcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Roadcoin
    // Mac: ~/Library/Application Support/Roadcoin
    // Unix: ~/.roadcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "roadcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "roadcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "roadcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Roadcoin will not work properly.");
