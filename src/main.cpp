...
#if defined(NDEBUG)
# error "Roadcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Roadcoin Signed Message:\n";
...
static const int64_t nTargetTimespan = 108 * 40; // roadcoin: 108 blocks (72 mins) [OLD WAS 6*60*3*20]
static const int64_t nTargetSpacing = 1 * 40; // roadcoin: 40 seconds
...
unsigned int GetNextWorkRequiredV1(const CBlockIndex* pindexLast, const CBlockHeader *pblock, int algo)
{
   ...
   int64_t nInterval = fInflationFixProtocol? (nTargetTimespanCurrent / nTargetSpacing) : (nTargetTimespanCurrent / (nTargetSpacing / 2));
    ...
    // roadcoin: This fixes an issue where a 51% attack can change difficulty at will.
